...
export default function DCACalculator() {
  ...
  useEffect(() => {
    ...
    if (amount || frequency || startDate || endDate) {
      const newFormData: DCAInput = {
        amount: amount ? parseFloat(amount) : 100,
        frequency: (frequency as Frequency) || 'weekly',
      ...
      if (amount && frequency && startDate && endDate) {
        setTimeout(() => {
          handleCalculateWithData(newFormData);
        }, 500);
      }
    }
  ...
  const generateShareableUrl = () => {
    ...
    return `${baseUrl}/tools/dca?${params.toString()}`;
  };
  ...
  const handleCalculate = async () => {
    await handleCalculateWithData(formData);
  };
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-6">
            ...
            <>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 border border-gray-800 text-gray-300 font-medium py-3 rounded-lg hover:border-brand-yellow hover:text-brand-yellow transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
      ...
      {/* Success Toast */}
      {showToast && (
        <div className="fixed bottom-4 right-4 bg-green-500 text-white px-6 py-3 rounded-lg shadow-lg flex items-center gap-3 animate-fade-in z-50">
          <svg className="w-5 h-5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"/>
          </svg>
          <div className="flex-1">
            <span className="font-medium block">{toastMessage}</span>
            {shareableUrl && (
              <span className="text-xs text-green-100 mt-1 block truncate max-w-xs">
                {shareableUrl}
              </span>
            )}
          </div>
