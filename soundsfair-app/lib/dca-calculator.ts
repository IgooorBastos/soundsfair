...
function generateTransactions(
  ...
  // First pass: collect all purchases
  while (currentDate <= end) {
    ...
    if (price) {
      ...
      transactions.push({
        ...
        portfolioValue: 0 // Will be calculated in second pass
      });
    }
  ...
  // Second pass: calculate portfolio value using the price at each point in time
  // This shows the historical value of your accumulated BTC at each date
  for (let i = 0; i < transactions.length; i++) {
    const transaction = transactions[i];
    const priceAtThisDate = transaction.price; // Use the price at this date for historical value
    transaction.portfolioValue = transaction.cumulativeUnits * priceAtThisDate;
  }
  ...
  const chartData = sortedDates.map(date => {
    ...
    results.forEach(result => {
      ...
      if (transactionIndex >= 0) {
        const transaction = result.transactions[transactionIndex];

        // Calculate total invested up to this point
        const totalInvestedUpToDate = result.transactions
          .slice(0, transactionIndex + 1)
          .reduce((sum, t) => sum + t.invested, 0);

        // Normalize: (current portfolio value / total invested) * 100
        // This shows the portfolio as a percentage of money invested
        const normalizedValue = (transaction.portfolioValue / totalInvestedUpToDate) * 100;
        point[result.asset] = normalizedValue;

        // Add total invested as baseline (always 100 in indexed mode)
        point[`${result.asset}_invested`] = 100;

        // Store raw values for absolute mode
        point[`${result.asset}_portfolioValue`] = transaction.portfolioValue;
        point[`${result.asset}_totalInvested`] = totalInvestedUpToDate;
      }
    });
